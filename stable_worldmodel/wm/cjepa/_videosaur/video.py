"""Temporal wrappers, vendored from martius-lab/videosaur (MIT License).

Source: videosaur/modules/video.py — verbatim aside from dropping the
YAML-driven `build()` function. ``MapOverTime`` applies a per-frame module
(the DINOv2 backbone + projection) independently across time; ``ScanOverTime``
applies a recurrent module (here, ``LatentProcessor`` wrapping SlotAttention)
across time, carrying slot state from one frame to the next.
"""
...
def merge_dict_trees(trees: List[Mapping], axis: int = 0):
    """Stack all leafs given a list of dictionary trees."""
    out = {}
    if len(trees) > 0:
        ref_tree = trees[0]
